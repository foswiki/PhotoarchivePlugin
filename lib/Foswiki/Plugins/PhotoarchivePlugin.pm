...
sub initPlugin
{
        ...
        Foswiki::Func::writeDebug( "- Foswiki::Plugins::${pluginName}::initPlugin( $web.$topic ) is OK" )
                if $debug;
...
sub attachedImages 
{
        ...
        if ( Foswiki::Func::checkAccessPermission( "VIEW", Foswiki::Func::getWikiUserName(), "", "$topic". 
                "Photoarchive", $web ) 
           )
        {
                $topiccontent = Foswiki::Func::readTopicText( $web, "$topic"."Photoarchive", "", "1" );
        ...
        {
                Foswiki::Func::writeDebug( "- Foswiki::Plugins::${pluginName}::attachedImages ".
...
{
        my $page = $_[0];
        my $attachment_url = Foswiki::Func::getPubUrlPath()."/"."$web"."/"."$topic"."Photoarchive/";
        my $images_per_col = Foswiki::Func::getPreferencesValue($pluginName."_THUMBSCOL") ||
                Foswiki::Func::getPluginPreferencesValue("THUMBSCOL") || "3";
        my $images_per_row = Foswiki::Func::getPreferencesValue($pluginName."_THUMBSROW") ||
                Foswiki::Func::getPluginPreferencesValue("THUMBSROW") || "4";
        my $images_per_page = Foswiki::Func::getPreferencesValue($pluginName."_DESCRIPTEDIMAGES") ||
                Foswiki::Func::getPluginPreferencesValue("DESCRIPTEDIMAGES") || "5";
        my $images_per_tpage = $images_per_row * $images_per_col;
        my $image_width = Foswiki::Func::getPreferencesValue($pluginName."_DESCRIPTEDWIDTH") ||
                Foswiki::Func::getPluginPreferencesValue("DESCRIPTEDWIDTH") || "250";
        my $image_height = Foswiki::Func::getPreferencesValue($pluginName."_DESCRIPTEDHEIGHT") ||
                Foswiki::Func::getPluginPreferencesValue("DESCRIPTEDHEIGHT") || "";
        my $detailedspace = Foswiki::Func::getPreferencesValue($pluginName."_DESCRIPTEDSPACE") ||
                Foswiki::Func::getPluginPreferencesValue( "DESCRIPTEDSPACE" ) || "5";
        ...
        if ( $pref_width ne $image_width || $pref_height ne $image_height )
        {
                if ( open(FH, "> $attachment_dir"."_"."$pluginName"."_"."descriptedprefs") )
                {
                        ...
                        close (FH);
                        Foswiki::Func::writeDebug( "- Foswiki::Plugins::${pluginName}::getDescriptedContent". 
        ...
        for ( my $i=0; $i <= $#image_file; $i++ )
        {
                        ...
                        ( $pref_width ne $image_width ) || ( $pref_height ne $image_height ) )
                {
                        ...
                        Foswiki::Sandbox->sysCommand( "$render_script ".
                                              '%SRC|F% %OUT|F% %LOG|F% %PAD|N% %SCL|U% ',
                                              SRC => $sourceimage,
                                              LOG => '/tmp/PA_log',
                                              SCL => $scalestr,
                                              PAD => 0,
                ...
                if ($Foswiki::cfg{Plugins}{LightboxPlugin}{Enabled} == 1) {
                    $img_td = '<td align="center">'."\n".'%LIGHTBOX{'.
                        'image="%ATTACHURLPATH%Photoarchive/'.$pfile->[$i].'" '.
                        'fullcaption="'.$pdescription->[$i].'<br>" '.
                        'thumbnail="'.$descripted.'"'.
                        'caption="'.$pheader->[$i].'" '.
                        '}% '.
                        '<br>'.
                    ...
                    $img_td = qq{<a href="?PhotoarchivePlugin_file=$pfile->[$i]&PhotoarchivePlugin_page=$virt_page&PhotoarchivePlugin_view=detailed" class="wikiLink"><img src="$descripted" alt="image: $pheader->[$i]" title="$pheader->[$i]" $himage_width $himage_height border="0"></a>};
                }
        ...
        {
                $pageback = $page - 1;
                $content .= qq(<a href="?PhotoarchivePlugin_page=$pageback&PhotoarchivePlugin_view=descripted" class="wikiLink">&lt;- back</a>);
        }
        else
        {
                $content .= qq(&lt;- back);
...
sub getDetailedContent 
{
        ...
        my $images_per_dpage = Foswiki::Func::getPreferencesValue($pluginName."_DESCRIPTEDIMAGES") ||
                Foswiki::Func::getPluginPreferencesValue("DESCRIPTEDIMAGES") || "5";
        my $image_width = Foswiki::Func::getPreferencesValue($pluginName."_DETAILEDWIDTH") ||
                Foswiki::Func::getPluginPreferencesValue("DETAILEDWIDTH") || "600";
        my $image_height = Foswiki::Func::getPreferencesValue($pluginName."_DETAILEDHEIGHT") ||
                Foswiki::Func::getPluginPreferencesValue("DETAILEDHEIGHT") || "";
        my $detailedspace = Foswiki::Func::getPreferencesValue($pluginName."_DETAILEDSPACE") ||
                Foswiki::Func::getPluginPreferencesValue( "DETAILEDSPACE" ) || "5";
        my (@image_file, @image_header, @image_description);
        my $pfile = \@image_file;
        my $pheader = \@image_header;
        my $pdescription = \@image_description;
        ...
        if ( $pref_width ne $image_width || $pref_height ne $image_height )
        {
                if ( open(FH, "> $attachment_dir"."_"."$pluginName"."_"."detailedprefs") )
                {
                        print FH "$image_width\n$image_height\n";
                        close (FH);
                        Foswiki::Func::writeDebug( "- Foswiki::Plugins::${pluginName}::getDetailedContent". 
        ...
        {
                $array_counter--;
                $content .= qq(<a href="?PhotoarchivePlugin_file=$pfile->[$array_counter]&PhotoarchivePlugin_page=$page&PhotoarchivePlugin_view=detailed" class="wikiLink">&lt;- back</a> --- );
                $array_counter++;
        }
        elsif ( $page >= 2 )
        {
                $pageback = $page - 1;
                $content .= qq(<a href="?PhotoarchivePlugin_nextfile=last&PhotoarchivePlugin_page=$pageback&PhotoarchivePlugin_view=detailed" class="wikiLink">&lt;- back</a> --- );
        }
        else
        {
                $content .= qq(&lt;- back --- );
        ...
        {
                $array_counter++;
                $content .= qq(<a href="?PhotoarchivePlugin_file=$pfile->[$array_counter]&PhotoarchivePlugin_page=$page&PhotoarchivePlugin_view=detailed" class="wikiLink">next -&gt;</a>);
                $array_counter--;
        }
        elsif ( $pagenext == 1 )
        {
                $pagenext = $page + 1;
                $content .= qq(<a href="?PhotoarchivePlugin_nextfile=first&PhotoarchivePlugin_page=$pagenext&PhotoarchivePlugin_view=detailed" class="wikiLink">next -&gt;</a>);
        }
        else
        {
                $content .= qq(next -&gt;);
...
sub getThumbsContent 
{
        ...
        my $images_per_dpage = Foswiki::Func::getPreferencesValue($pluginName."_DESCRIPTEDIMAGES") ||
                Foswiki::Func::getPluginPreferencesValue("DESCRIPTEDIMAGES") || "5";
        my $image_width = Foswiki::Func::getPreferencesValue($pluginName."_THUMBSWIDTH") ||
                Foswiki::Func::getPluginPreferencesValue("THUMBSWIDTH") || "";
        my $image_height = Foswiki::Func::getPreferencesValue($pluginName."_THUMBSHEIGHT") ||
                Foswiki::Func::getPluginPreferencesValue("THUMBSHEIGHT") || "120";
        my $thumbsspace = Foswiki::Func::getPreferencesValue($pluginName."_THUMBSSPACE") ||
                Foswiki::Func::getPluginPreferencesValue( "THUMBSSPACE" ) || "5";
        my (@image_file, @image_header, @image_description);
        my $pfile = \@image_file;
        my $pheader = \@image_header;
        my $pdescription = \@image_description;
        ...
        if ( $pref_width ne $image_width || $pref_height ne $image_height )
        {
                if ( open(FH, "> $attachment_dir"."_"."$pluginName"."_"."thumbsprefs") )
                {
                        print FH "$image_width\n$image_height\n";
                        close (FH);
                        Foswiki::Func::writeDebug( "- Foswiki::Plugins::${pluginName}::getThumbsContent". 
        ...
        for ( my $i=0; $i <= $#image_file; $i++ )
        {
                        ...
                        ( $pref_width ne $image_width ) || ( $pref_height ne $image_height ) )
                {
                        ...
                        Foswiki::Sandbox->sysCommand( "$render_script %SRC|F% %OUT|F% %LOG|F% %PAD|N% %SCL|U% ",
                                              SRC => $sourceimage,
                                              OUT => $sourcethumb,
                                              LOG => '/tmp/PA_log',
                                              PAD => $pnmwidth,
                ...
                if ($Foswiki::cfg{Plugins}{LightboxPlugin}{Enabled} == 1) {
                 ...
                 $tag = qq{<td align="center"><a href="?PhotoarchivePlugin_file=$pfile->[$i]&PhotoarchivePlugin_page=$page&PhotoarchivePlugin_view=detailed" class="wikiLink"><img src="$thumb" alt="image: $pheader->[$i]" title="$pheader->[$i]" $himage_width $himage_height border="0"></a></td>\n};
                }
        ...
        {
                $pageback = $page - 1;
                $content .= qq(<a href="?PhotoarchivePlugin_page=$pageback&PhotoarchivePlugin_view=thumbs" class="wikiLink">&lt;- back</a>);
        }
        else
        {
                $content .= qq(&lt;- back);
        ...
        {
                $pagenext = $page + 1;
                $content .= qq(<a href="?PhotoarchivePlugin_page=$pagenext&PhotoarchivePlugin_view=thumbs" class="wikiLink">next -&gt;</a>);
        }
        else
        {
                $content .= qq(next -&gt;);
...
sub startPhotoarchive
{
        ...
        Foswiki::Func::writeDebug( "- Foswiki::Plugins::${pluginName}::startPhotoarchive". 
        ...
        if ( $cgi_query ) 
        {
                ...
                $view = $cgi_query->param('PhotoarchivePlugin_view') || 
                        Foswiki::Func::getPluginPreferencesValue( "DEFAULTVIEW" ) || "thumbs";
        ...
        $attachment_dir = Foswiki::Func::getPubDir()."/"."$web"."/"."$topic"."Photoarchive/";
...
sub startRandom
{
        ...
        my $rndtopic = Foswiki::Func::getPreferencesValue($pluginName."_RANDOMTOPIC") ||
                Foswiki::Func::getPluginPreferencesValue("RANDOMTOPIC") ||
                $topic;
        my $rndweb = Foswiki::Func::getPreferencesValue($pluginName."_RANDOMWEB") ||
                Foswiki::Func::getPluginPreferencesValue("RANDOMWEB") ||
                $web;
        my $attachment_url = Foswiki::Func::getPubUrlPath()."/".$rndweb."/".$rndtopic."/";
        my $rnd_dir = Foswiki::Func::getPubDir()."/".$web."/".$topic."/";
        my $rnd_url = Foswiki::Func::getPubUrlPath()."/".$web."/".$topic."/";
        my $images_per_page = 1;
        my $image_width = Foswiki::Func::getPreferencesValue($pluginName."_RANDOMWIDTH") ||
                Foswiki::Func::getPluginPreferencesValue("RANDOMWIDTH") ||
                "";
        my $image_height = Foswiki::Func::getPreferencesValue($pluginName."_RANDOMHEIGHT") ||
        ...
        unless ( Foswiki::Func::checkAccessPermission( "VIEW", Foswiki::Func::getWikiUserName(), "",
                $rndtopic, $rndweb ) 
           )
        {
                Foswiki::Func::writeDebug( "- Foswiki::Plugins::${pluginName}::startRandom ".
        ...
        $attachment_dir = Foswiki::Func::getPubDir()."/".$rndweb."/".$rndtopic."Photoarchive/";
